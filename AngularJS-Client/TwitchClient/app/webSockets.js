    ...
    .factory('websocketUrl', [
        'guid', '$log', function(guid, $log) {
            ...
            return {
                ...
                port: defaultPort,
                url: function() {
                    return 'ws://' + this.host + ':' + this.port + '/ws/' + id;
                }
            };
        }
    ])
    .service('$webSocketConnectionFactory', [
        '$q', '$timeout', '$log', '$rootScope', function($q, $timeout, $log, $rootScope) {
            var factory = {};

            factory.connections = [];
            factory.connectionsByName = {};
            factory.onCreatedPromises = {};
            factory.onCreated = function(name) {
                var deferred = $q.defer();
                if (factory.connectionsByName[name]) {
                    deferred.resolve(factory.connectionsByName[name]);
                } else {
                    factory.onCreatedPromises[name] = deferred;
                }
                return deferred.promise;
            };

            factory.create = function(name, websocketUrl) {
                var me = {};
                var listeners = [];
                ...
                me.connect = function() {
                    if (!me.isConnecting && !me.isConnected) {
                        ...
                        me.ws = new WebSocket(websocketUrl);
                        setHandlers(me.ws);
                    }
                };

                var promise = factory.onCreatedPromises[name];
                if (promise) {
                    factory.onCreatedPromises[name] = null;
                    promise.resolve(me);
                }

                factory.connections.push({ name: name, connection: me });
                factory.connectionsByName[name] = me;
